...
class Species:
 ...
 class rpReader():
    ...
    def _readPaths(cache, rp2paths_pathways):
        """Function that reads the pathway output of rp2paths
    ...
    def _gatherSpeciesInfo(cache, meta, rp_strc, pubchem_search):
        ...
        # xref
        try:
            xref = cache.cid_xref[meta]
        except KeyError:
            xref = {}
        species = Species(None, None, None, xref)
        ###### Try to recover the structures ####
        pubchem = Species(None, None, None, {})
        # inchi
        try:
            species.inchi = rp_strc[meta]['inchi']
            if not species.xref and pubchem_search:
                try:
                    pubchem.inchi = cache._pubchem_speciescies[species.inchi]['inchi']
                    pubchem.inchikey = cache._pubchem_speciescies[species.inchi]['inchikey']
                    pubchem.smiles = cache._pubchem_speciescies[species.inchi]['smiles']
                    pubchem.xref = cache._pubchem_speciescies[species.inchi]['xref']
                except KeyError:
                    pubres = _pubchemStrctSearch(species.inchi, 'inchi')
                    if not chemName:
                        chemName = pubres['name']
                    if 'chebi' in pubres['xref']:
                        try:
                            species.xref = cache.cid_xref[cache.chebi_cid[pubres['xref']['chebi'][0]]]
                        except KeyError:
                            pass
        ...
        try:
            ...
            species.inchikey = rp_strc[meta]['inchikey']
            if not species.xref and pubchem_search:
                pubres = _pubchemStrctSearch(species.inchikey, 'inchikey')
                if not chemName:
                    chemName = pubres['name']
                if 'chebi' in pubres['xref']:
                    try:
        ...
        try:
            ...
            species.smiles = rp_strc[meta]['smiles']
            if not species.xref and pubchem_search:
                pubres = _pubchemStrctSearch(species.smiles, 'smiles')
                if not chemName:
                    chemName = pubres['name']
                if 'chebi' in pubres['xref']:
                    try:
        ...
        if not species.inchi:
            species.inchi = pubchem.inchi
        if not species.inchikey:
            species.inchikey = pubchem.inchikey
        if not species.smiles:
            species.smiles = pubchem.smiles
        if not species.xref:
            species.xref = pubchem.xref
        if pubchem.inchi:
            cache._pubchem_speciescies[pubchem.inchi] = {'inchi': pubchem.inchi, 'smiles': pubchem.smiles, 'inchikey': pubchem.inchikey, 'xref': pubchem.xref}
        return (chemName, species)
    ...
    def rp2pathsToSBML(self,
        ...
        rp_paths = self._readPaths(cache, rp2paths_pathways)
        sink_species = []
        ...
        for pathNum in rp_paths:
            ...
            for comb_path in list(itertools_product(*[[(i,y) for y in rp_paths[pathNum][i]] for i in rp_paths[pathNum]])):
                ...
                rpsbml.genericModel(
                        'RetroPath_Pathway_'+str(path_id)+'_'+str(alt_path_num),
                ...
                for meta in all_meta:
                    (chemName, spe) = self._gatherSpeciesInfo(cache, meta, rp_strc, pubchem_search)
                    if chemName:
                        chemName = chemName.replace("'", "")
                    # pass the information to create the species
                    rpsbml = self._addSpecies(rpsbml, meta, sink_molecules, compartment_id, chemName, spe, species_group_id, sink_species_group_id)
            ...
            for rpsbml in list_rpsbml:
                rpsbml.writeSBML(outFolder)
        return True


    def _addSpecies(rpsbml, meta, sink_molecules, compartment_id, chemName, spe, species_group_id, sink_species_group_id):
