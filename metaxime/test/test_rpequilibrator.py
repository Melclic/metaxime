...
class TestRPEquilibrator(unittest.TestCase):
    ...
    def test_makeReactionStr(self):
        self.assertEqual(rpeq._makeReactionStr(rpeq.model.getReaction('RP1')), '1.0 CHEBI:6280 + 1.0 CHEBI:5584 <=> 1.0 CHEBI:8650 + 1.0 CHEBI:3283 ')
        self.assertEqual(rpeq._makeReactionStr(rpeq.model.getReaction('RP1'), 'name'), '1.0 L-ornithine + 1.0 H(+) <=> 1.0 putrescine + 1.0 CO2 ')
        self.assertEqual(rpeq._makeReactionStr(rpeq.model.getReaction('RP1'), 'id'), '1.0 CMPD_0000000004 + 1.0 MNXM1 <=> 1.0 TARGET_0000000001 + 1.0 MNXM13 ')

    def test_speciesCmpQuery(self):
        res = self.rpeq._speciesCmpQuery(self.rpeq.model.getSpecies('MNXM89557__64__MNXC3'))
        self.assertEqual(res[0], data['speciescmpquery']['mu'])
        self.assertCountEqual(res[1], data['speciescmpquery']['sigma'])
    ...
    def test_pathway(self):
        res = rpeq.pathway(write_results=False)
